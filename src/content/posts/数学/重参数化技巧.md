...
> 得到$\epsilon_{i}<\epsilon_{i}^{p_{2}/p_{1}}\leq 1$,而由于$\epsilon_{i}\sim U[0,1]$,所以成立的概率为$\epsilon^{p_{2}/p_{1}}$,如果需要同时满足,那么就可以得到概率为$\epsilon_{1}^{1/p_{1}-1}$.对所有的$\epsilon_{1}$求平均就是$\int^{1}_{0}\epsilon_{1}^{1/p_{1}-1}\epsilon_{1}=p_{1}$,这就是类别 1 出现的概率.至此我们完成了采样过程的证明
